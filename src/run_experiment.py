#!/usr/bin/env python3
"""
Run random permutations of LLVM passes on benchmarks and measure code size.
Compares against -Oz baseline.

Usage:
    python3 src/run_experiment.py --benchmarks-dir ./benchmarks/compiled --num-iterations 2500
"""
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for bench_name, bench_path in benchmarks:
            ...
            for i in range(num_iterations):
                ...
                perm = SHUFFLEABLE_PASSES.copy()
                random.shuffle(perm)

                opt_size = run_single_permutation(opt_path, bench_path, perm, tmpdir)

                if opt_size is None:
                    # opt crashed — record as failed
                    results.append({
                        "benchmark": bench_name,
                        "pass_sequence": [p.lstrip("-") for p in perm],
                        "optimized_size": None,
...
def main():
    parser = argparse.ArgumentParser(
        description="Run random pass permutation experiment"
    )
    parser.add_argument("--benchmarks-dir", required=True,
                        help="Directory containing .bc benchmark files")
    parser.add_argument("--num-iterations", type=int, default=2500,
                        help="Number of random permutations per benchmark (default: 2500)")
    parser.add_argument("--output", default="raw_results.json",
                        help="Output JSON file (default: raw_results.json)")
